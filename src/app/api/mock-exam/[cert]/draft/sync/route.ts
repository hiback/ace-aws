...
import { parseMockExamSyncPayload } from '@/server/mock-exam-sync/contract'
import { syncAccountBackedMockExamDraft } from '@/server/mock-exam-sync/service'
...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!hasJsonContentType(request)) {
    return error('unsupported_media_type', 'Mock Exam Draft Sync requires application/json', 415)
  }
  ...
  const parsed = parseMockExamSyncPayload(cert, payload)
  if ('error' in parsed) return error(parsed.error.code, parsed.error.message, 400)

  const result = await syncAccountBackedMockExamDraft(userId, parsed)
