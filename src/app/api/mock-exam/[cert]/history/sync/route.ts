...
import { parseMockExamHistorySyncPayload } from '@/server/mock-exam-sync/contract'
import { syncAccountBackedMockExamHistory } from '@/server/mock-exam-sync/service'
...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!hasJsonContentType(request)) {
    return error('unsupported_media_type', 'Mock Exam History Sync requires application/json', 415)
  }
  ...
  const parsed = parseMockExamHistorySyncPayload(cert, payload)
  if ('error' in parsed) return error(parsed.error.code, parsed.error.message, 400)

  const result = await syncAccountBackedMockExamHistory(userId, parsed)
