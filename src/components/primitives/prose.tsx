...
export type ProseVariant = 'stem' | 'option' | 'explanation'
...
const paragraphClassByVariant: Record<ProseVariant, string> = {
  stem: 'text-body text-ink leading-[1.65] mb-2.5 last:mb-0',
  option: 'text-option text-ink leading-[1.55] mb-1.5 last:mb-0',
  explanation: 'text-option text-ink-soft leading-[1.65] mb-2.5',
}

const wrapperClassByVariant: Record<ProseVariant, string> = {
  stem: 'text-body text-ink leading-[1.65]',
  option: 'text-option text-ink leading-[1.55]',
  explanation: 'text-option text-ink-soft leading-[1.65]',
}

function buildComponents(variant: ProseVariant): Components {
  return {
    p: ({ children }) => <p className={paragraphClassByVariant[variant]}>{children}</p>,
    h1: ({ children }) => (
...
export function Prose({ source, variant, className }: ProseProps) {
  return (
    <div className={[wrapperClassByVariant[variant], className ?? ''].join(' ').trim()}>
      <ReactMarkdown
